  ...
  try {
    ...
    for (const row of res.rows) {
      try {
        ...
        // No receipt. If the record is older than 5 minutes the tx was dropped
        // from the mempool (e.g. node restart) and will never mine — mark failed
        // so the ledger isn't stuck on a misleading "submitted".
        const ageRes = await pool.query(
          `SELECT (NOW() - recorded_at) > INTERVAL '5 minutes' AS stale FROM onchain_trade_records WHERE id = $1`,
          [row.id]
        );
        if (ageRes.rows[0]?.stale) {
          await pool.query(
            `UPDATE onchain_trade_records
               SET status = 'failed', error_message = 'Transaction dropped (never mined — submitted before fee config or lost on node restart)', updated_at = NOW()
             WHERE id = $1`,
            [row.id]
          );
          updated++;
        }
        // else: recent, still legitimately pending — leave as 'submitted'
      } catch (e) {
