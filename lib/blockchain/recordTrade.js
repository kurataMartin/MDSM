 ...
 */
// Returns true when the configured RPC URL is a private/loopback address that
// is unreachable from a cloud host (Vercel, AWS Lambda, etc.).
function isPrivateRpc() {
  const url = process.env.BLOCKCHAIN_RPC_URL || "";
  return (
    url.includes("localhost")  ||
    url.includes("127.0.0.1") ||
    url.match(/https?:\/\/172\.(1[6-9]|2\d|3[01])\./) || // 172.16–31.*
...
export async function recordTradeOnChain(orderRow, symbol = "") {
  // Skip silently when the blockchain node is on a private network and we are
  // running in a cloud environment (Vercel).  The trade is still valid in the
  // DB; the on-chain record will be created when the app runs locally or when
  // a public RPC endpoint is configured.
  if (isPrivateRpc() && (process.env.VERCEL === '1' || process.env.AWS_LAMBDA_FUNCTION_NAME)) {
    console.warn(
      `[CHAIN] Skipping on-chain recording for order #${orderRow.id}: ` +
      `BLOCKCHAIN_RPC_URL (${process.env.BLOCKCHAIN_RPC_URL}) is a private address ` +
      `unreachable from this cloud environment. Set a public RPC URL to enable on-chain recording.`
    );
    return null;
  }
