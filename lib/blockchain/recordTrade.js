...
// Sanitise the RPC URL: strip an accidental "KEY = " prefix, whitespace, and
// surrounding quotes that can sneak in when pasting into a dashboard env field.
// Without this, a value like "blockchain_rpc_url = http://host:8545" makes
// ethers throw UNSUPPORTED_OPERATION ("unsupported protocol blockchain_rpc_url = http").
function sanitizeRpcUrl(raw) {
  let url = (raw || "").trim().replace(/^["']|["']$/g, "");
  // Drop anything before the protocol (handles "BLOCKCHAIN_RPC_URL = http://…")
...
function getClient() {
  const rpcUrl          = sanitizeRpcUrl(process.env.BLOCKCHAIN_RPC_URL);
  ...
  if (!/^https?:\/\//i.test(rpcUrl)) {
    throw new Error(`BLOCKCHAIN_RPC_URL is malformed: "${process.env.BLOCKCHAIN_RPC_URL}" — must start with http:// or https://`);
  }
...
export async function recordTradeOnChain(orderRow, symbol = "") {
  ...
  try {
    ...
    // Persist 'submitted' + hash IMMEDIATELY. On serverless (Vercel) the
    // function can be frozen/killed right after the HTTP response returns,
    // before the wait below resolves — so we must record the hash now. A later
    // reconciliation (on dashboard view) promotes it to 'confirmed'.
    await saveChainRecord(orderRow.id, tx.hash, 0, "submitted", "Awaiting confirmation");
...
/**
 * Re-check transactions stuck in 'submitted' and promote them to 'confirmed'
 * (or 'failed') by fetching their receipt. Handles confirmations that landed
 * after the inline 60s wait window. Safe to call frequently; processes a small
 * batch per invocation. Returns the number of records updated.
 */
export async function reconcileSubmittedChainTxns(limit = 25) {
  let provider = null;
  let updated = 0;
  try {
    const res = await pool.query(
      `SELECT id, tx_hash FROM onchain_trade_records
       WHERE status = 'submitted' AND tx_hash IS NOT NULL AND tx_hash NOT LIKE 'pending-%'
    ...
    for (const row of res.rows) {
      try {
        ...
        if (receipt) {
          ...
          await pool.query(
            `UPDATE onchain_trade_records
               SET status = $2, block_number = $3, error_message = $4, updated_at = NOW()
             WHERE id = $1`,
            [row.id, ok ? "confirmed" : "failed", Number(receipt.blockNumber) || 0,
        ...
        // getTransaction returns the tx object if the node still knows it
        // (blockNumber null = pending), or null if it was dropped/never seen.
        const txn = await provider.getTransaction(row.tx_hash).catch(() => null);
        if (txn) {
          // Node still has it — genuinely pending, will mine soon. Leave as 'submitted'.
          continue;
        }

        // Node doesn't know the tx. Only mark dropped if it's been long enough
        // (>15 min) that it can't simply be propagation lag.
        const ageRes = await pool.query(
          `SELECT (NOW() - recorded_at) > INTERVAL '15 minutes' AS stale FROM onchain_trade_records WHERE id = $1`,
          [row.id]
        );
        if (ageRes.rows[0]?.stale) {
          await pool.query(
            `UPDATE onchain_trade_records
               SET status = 'failed', error_message = 'Transaction not found on-chain (dropped or never propagated)', updated_at = NOW()
             WHERE id = $1`,
            [row.id]
          );
          updated++;
        ...
        console.warn(`[CHAIN] reconcile failed for tx ${row.tx_hash}:`, e.message);
      }
    }
    return updated;
  } catch (err) {
    console.error("[CHAIN] reconcileSubmittedChainTxns failed:", err.message);
    return updated;
  } finally {
    try { provider?.destroy(); } catch (_) {}
  }
