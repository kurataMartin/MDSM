...
function parseFields(rawText) {
  ...
  // Registered address — prefer a CONTENT match (a distinctive street/PO-box
  // address) since label position is unreliable in OCR'd multi-column layouts.
  let addr =
    text.match(/\b([A-Z][A-Za-z]+(?:\s+[A-Z][A-Za-z]+)*\s+Road,\s*[A-Za-z]+\s*\d+,?\s*Lesotho)/i)?.[1] ||
    text.match(/\b(P\.?\s*O\.?\s*Box\s*\d+,\s*[A-Za-z]+\s*\d+,?\s*Lesotho)/i)?.[1] ||
    between(["PHYSICAL ADDRESS", "REGISTERED OFFICE ADDRESS", "REGISTERED OFFICE", "REGISTERED ADDRESS", "PRINCIPAL PLACE OF BUSINESS"]);
  if (addr && !startsWithLabel(addr)) set("registered_address", addr, 0.85);
