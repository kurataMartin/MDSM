...
function parseFields(rawText) {
  // Keep original text with line breaks
  const text = rawText;
  ...
  // Method 1: Look for "COMPANY NAME" at the beginning of a line or after newline
  let companyNameMatch = text.match(/(?:^|\n)\s*COMPANY\s+NAME\s+([^\n]+)/i);
  
  // Method 2: Look for "COMPANY NAME" with colon
  if (!companyNameMatch) {
    companyNameMatch = text.match(/COMPANY\s+NAME\s*:\s*([^\n]+)/i);
  }
  
  // Method 3: Look for "COMPANY NAME" anywhere but capture until next label or newline
  if (!companyNameMatch) {
    companyNameMatch = text.match(/COMPANY\s+NAME\s+([A-Z][A-Za-z0-9&.,'\-\s]{3,150}?(?:\(Pty\)\s+Ltd|Limited|Ltd|PLC|Inc))/i);
  }
  ...
  if (companyNameMatch && companyNameMatch[1]) {
    ...
    // Fallback: look for pattern with company suffix
    const fallbackMatch = text.match(/([A-Z][A-Za-z0-9&.,'\-\s]{3,100}?(?:\(Pty\)\s+Ltd|Limited|Ltd|PLC|Inc))/i);
    if (fallbackMatch) {
      set("company_name", fallbackMatch[1].trim(), 0.7);
    }
  }

  // ── REGISTRATION NUMBER EXTRACTION ───────────────────────────────────────
  let regMatch = text.match(/(?:^|\n)\s*COMPANY\s+REGISTRATION\s+NUMBER\s+([^\n]+)/i);
  if (!regMatch) {
    regMatch = text.match(/COMPANY\s+REGISTRATION\s+NUMBER\s*:\s*([^\n]+)/i);
  }
  if (!regMatch) {
    regMatch = text.match(/COMPANY\s+REGISTRATION\s+NUMBER\s+(CR[-\s]?\d{2,4}\/?\d{2,6})/i);
  }
  if (!regMatch) {
  ...
  if (!dateMatch) {
    dateMatch = text.match(/DATE\s+OF\s+INCORPORATION\s*:\s*([^\n]+)/i);
  }
  if (!dateMatch) {
    dateMatch = text.match(/DATE\s+OF\s+INCORPORATION\s+(\d{1,2}\s+(?:January|February|March|April|May|June|July|August|September|October|November|December)\s+\d{4})/i);
  }
  if (!dateMatch) {
  ...
  let typeMatch = text.match(/(?:^|\n)\s*COMPANY\s+TYPE\s+([^\n]+)/i);
  if (!typeMatch) {
    typeMatch = text.match(/COMPANY\s+TYPE\s*:\s*([^\n]+)/i);
  }
  if (!typeMatch) {
    typeMatch = text.match(/COMPANY\s+TYPE\s+(Private Company Limited by Shares|Public Company|Company Limited by Guarantee)/i);
  }
  if (!typeMatch) {
  ...
  if (!addressMatch) {
    addressMatch = text.match(/REGISTERED\s+OFFICE\s+ADDRESS\s+(Kingsway Road|P\.?O\.?Box)/i);
  }
  ...
  let directorsMatch = text.match(/(?:^|\n)\s*DIRECTORS\s+([^\n]+)/i);
  if (!directorsMatch) {
    directorsMatch = text.match(/DIRECTORS\s*:\s*([^\n]+)/i);
  }
  if (!directorsMatch) {
    directorsMatch = text.match(/DIRECTORS\s+([A-Za-z\s;]+)/i);
  }
  ...
  if (!capitalMatch) {
    capitalMatch = text.match(/AUTHORISED\s+SHARE\s+CAPITAL\s*:\s*([^\n]+)/i);
  }
  if (!capitalMatch) {
    capitalMatch = text.match(/AUTHORISED\s+SHARE\s+CAPITAL\s+(M\s*[\d,]+(?:\.\d{2})?\s*\([^)]+shares[^)]+\))/i);
  }
  if (!capitalMatch) {
