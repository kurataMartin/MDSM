...
function parseFields(rawText) {
  // Normalise whitespace but keep newlines as separators.
  const text = rawText.replace(/ /g, " ").replace(/[ \t]+/g, " ");

  const esc = (s) => s.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");

  // Find a value by label anywhere in the text. Tolerates:
  //   "LABEL: value", "LABEL value", and "LABEL\nvalue".
  // Captures the remainder of the value line and rejects ALL-CAPS lines
  // (which are themselves labels, not values).
  const byLabel = (variants) => {
    for (const lab of variants) {
      const re = new RegExp(esc(lab) + "[ \\t]*:?[ \\t]*\\r?\\n?[ \\t]*([^\\r\\n]+)", "i");
      const m = text.match(re);
      if (m && m[1]) {
        const v = m[1].trim().replace(/\s{2,}/g, " ");
        if (v && !/^[A-Z][A-Z ./]{4,}$/.test(v) && !/^\(.*\)$/.test(v)) return v;
      }
    }
    return null;
  };
    ...
    if (file?.file_data && /pdf/i.test(mime)) {
      try {
        ...
        if (text && text.replace(/\s/g, "").length > 40) {
          ...
          // Trust the real extraction whenever it found the company name OR the
          // registration number (the core identity fields).
        ...
        // Malformed/encrypted PDF (e.g. "bad XRef entry") — don't fail the whole
        // flow; fall through to the sample fallback so the issuer can still proceed.
        console.warn(`[REG] PDF parse failed for draft ${draftId}: ${pdfErr.message} — using fallback sample`);
      }
    }

    // Fallback for image scans / text-less PDFs / unreadable PDFs / low-match results.
    if (!result) {
