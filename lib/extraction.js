...
function parseFields(rawText) {
  ...
  if (cap && !startsWithLabel(cap)) {
    ...
    const amountM = cap.match(/([\d,]{4,}(?:\.\d+)?)/);
    const amount  = amountM ? num(amountM[1]) : null;
    const sharesM = cap.match(/([\d,]{4,})\s*(?:ordinary\s+)?shares/i);
    const totalShares = sharesM ? num(sharesM[1]) : amount;
    if (totalShares != null) set("authorized_shares", totalShares, 0.82);

    const perShareM = cap.match(/of\s*M?\s*([\d,]+(?:\.\d+)?)\s*(?:each|per\s+share)/i)
                   || cap.match(/M\s*([\d,]+(?:\.\d+)?)\s*(?:each|per\s+share)/i);
    let price = perShareM ? num(perShareM[1]) : null;
    if (price == null && amount != null && totalShares) {
      price = Math.round((amount / totalShares) * 100) / 100;
    }
    if (price != null && isFinite(price) && price > 0) set("price_per_share", price, 0.78);
  }
