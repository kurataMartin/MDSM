...
export async function approveKYC(userId, actorId = 5) {
  ...
  try {
    ...
    // Notify the user (best-effort — never block approval on this)
    try {
      await query(
        ...
        [
          numericUserId,
          "KYC Approved",
          "Your identity verification has been approved. You now have full access to your dashboard and can begin trading.",
        ]
      );
    } catch (notifyErr) {
      console.warn(`[approveKYC] Notification insert failed for user ${numericUserId}:`, notifyErr.message);
    }
    ...
    try {
      await query(
        ...
        [
          numericId,
          "KYC Rejected",
          "Your identity verification was not approved. Please review your details and resubmit your KYC documents.",
        ]
      );
    } catch (notifyErr) {
      console.warn(`[rejectKYC] Notification insert failed for user ${numericId}:`, notifyErr.message);
    }
  ...
  try {
    const result = await query(`
      SELECT
        ...
        a.target_table,
        CASE
          WHEN a.target_table IS NOT NULL AND a.target_table <> ''
            THEN initcap(replace(a.action, '_', ' ')) || ' on ' || a.target_table
...
export async function getOnchainTradeRecords() {
  try {
    // Self-heal: promote confirmed txns and mark dropped ones before reading.
    try {
      const { reconcileSubmittedChainTxns } = await import("./blockchain/recordTrade");
      await reconcileSubmittedChainTxns(25);
    } catch (e) {
      console.warn("[getOnchainTradeRecords] reconcile skipped:", e.message);
    }
...
// Public platform stats for the landing page — all real values from the DB.
export async function getPlatformStats() {
  try {
    const [secRes, volRes, userRes] = await Promise.all([
      query(`SELECT COUNT(*)::int AS n FROM securities WHERE approved = true OR approved = 't'`),
      query(`
        SELECT COALESCE(SUM(total), 0)::numeric AS vol
        FROM trades
        WHERE status IN ('settled','filled')
          AND created_at >= NOW() - INTERVAL '24 hours'
      `),
      query(`SELECT COUNT(*)::int AS n FROM users`),
    ]);

    const dailyVolume = Number(volRes.rows[0]?.vol || 0);
    // Fallback: if no trades in the last 24h, show all-time volume so the
    // figure is never a flat zero on a quiet day.
    let volume = dailyVolume;
    if (volume === 0) {
      const allTime = await query(
        `SELECT COALESCE(SUM(total),0)::numeric AS vol FROM trades WHERE status IN ('settled','filled')`
      );
      volume = Number(allTime.rows[0]?.vol || 0);
    }

    return {
      listedSecurities: Number(secRes.rows[0]?.n || 0),
      dailyVolume:      volume,
      registeredUsers:  Number(userRes.rows[0]?.n || 0),
    };
  } catch (err) {
    console.error("getPlatformStats failed:", err.message);
    return { listedSecurities: 0, dailyVolume: 0, registeredUsers: 0 };
  }
}
...
// Resolve the absolute base URL for a fetch.
// Client-side: always window.location.origin (correct everywhere).
// Server-side: prefer Vercel's own deployment URL over NEXT_PUBLIC_BASE_URL,
//   because NEXT_PUBLIC_BASE_URL is often left as http://localhost:3000 in env
//   vars — which causes ECONNREFUSED 127.0.0.1:3000 inside a Vercel function.
function getServerBaseUrl() {
  // Vercel injects these automatically at runtime
  const prodUrl = process.env.VERCEL_PROJECT_PRODUCTION_URL; // e.g. mdsm-aw9w.vercel.app
  const depUrl  = process.env.VERCEL_URL;                    // current deployment host
  if (prodUrl) return `https://${prodUrl}`;
  if (depUrl)  return `https://${depUrl}`;

  const envBase = process.env.NEXT_PUBLIC_BASE_URL || "";
  // Ignore a localhost value when actually running on Vercel
  if (process.env.VERCEL === "1" && /localhost|127\.0\.0\.1/.test(envBase)) return "";
  return envBase;
}

function getApiUrl(path) {
  const base = typeof window === "undefined" ? getServerBaseUrl() : window.location.origin;
...
export async function getPendingOrders(_brokerUserId = null) {
  // All brokers can see and execute all pending orders.
  // The broker_assignments filter was too restrictive — if the table is empty
  // or a broker has no explicit assignments, no orders appeared at all.
  try {
    const result = await pool.query(`
      SELECT
        o.id,
...
// Run once per process lifetime — not on every order execution.
let _brokerFeeColumnsEnsured = false;
async function ensureBrokerFeeColumns() {
  if (_brokerFeeColumnsEnsured) return;
  _brokerFeeColumnsEnsured = true; // set early so concurrent calls don't double-run
  ...
  // One-time repair: recompute available_tokens where NULL or out-of-sync
  await query(`
    UPDATE securities s
    SET available_tokens = GREATEST(
          s.total_supply - COALESCE(
            (SELECT SUM(h.quantity) FROM holdings h WHERE h.security_id = s.id), 0
          ), 0
        )
...
// Always returns { success, message?, ... } — never throws — so Next.js
// production mode cannot redact the error reason before it reaches the UI.
export async function executeOrder(orderId, brokerId) {
  ...
  return await withTransaction(async (client) => {
    ...
    // ── 11. SAVE TO BLOCKCHAIN — fire-and-forget ─────────────────────
    // The DB transaction is already committed at this point.
    // Blockchain confirmation takes 20–60 s; awaiting it blocks the HTTP
    // response and will hit Vercel Hobby's 10-second function timeout.
    // We fire the recording in the background and return success immediately.
    // The onchain_trade_records table is updated asynchronously.
    const orderRowForChain = { ...orderRow };
    recordTradeOnChain(orderRowForChain).then((txHash) => {
      console.log(`[EXECUTE] On-chain tx for order #${orderId}: ${txHash || 'skipped'}`);
      if (txHash) {
        // Best-effort update — not awaited, non-blocking
        query(`UPDATE orders SET onchain_tx_hash = $1 WHERE id = $2`, [txHash, orderId])
          .catch((e) => console.warn(`[EXECUTE] Hash save failed for #${orderId}:`, e.message));
      }
    }).catch((e) => {
      console.error(`[EXECUTE] Blockchain recording failed for #${orderId}:`, e.message);
    });
    ...
    return {
      ...
      price: prc,
      total,
      txHash: null, // hash not yet available — recorded async
    };
  });
  } catch (err) {
    // Return error as a value so Next.js production mode cannot redact the message
    console.error(`[executeOrder] Failed for order #${orderId}:`, err.message);
    return { success: false, message: err.message || "Order execution failed" };
  }
}
...
`, [
  ...
  // Step 2: Blockchain recording — fire-and-forget, does not block response
  if (result?.success && finalOrderRow) {
    safelyRecordOnChain(finalOrderRow).catch((e) => {
      console.error(`[PLACE&EXECUTE] Blockchain failed for #${finalOrderRow.id}:`, e.message);
    });
  }
  ...
  try {
    ...
    const sql = `
      SELECT
        ...
        created_at AS uploaded_at,
        kyc_record_id,
        COALESCE(upload_error, NULL) AS upload_error
      FROM kyc_documents
      WHERE user_id = $1
      ORDER BY created_at DESC
    `;

    const result = await db.query(sql, [Number(userId)]).catch(async () => {
      // Older schema without upload_error column
      return db.query(`
        SELECT id, document_type, document_number, document_url, status,
               created_at AS uploaded_at, kyc_record_id
